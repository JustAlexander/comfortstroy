...
const FormFoto = () => {
  ...
  const [buttonText, setButtonText] = useState(`Записаться на просмотр`);
  ...
  const handleSubmit = async (e) => {
    ...
    if (isValidForm) {
      ...
      if (error) {
        ...
        setButtonText(`Записаться на просмотр`);
        // Reset form fields
        setPhone(``);
        setName(``);
        return;
    ...
    <section className={tw(`overflow-hidden`)}>
      <div className={tw(`max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 bg-white`)}>
        <div className={tw(`sm:mb-16 text-center`)}>
          <p className={tw(`text-base text-yellow-600 font-semibold tracking-wide uppercase`)}>Оцените качество работ в живую</p>
          <h2 className={tw(`mt-2 pb-4 text-4xl lg:text-7xl font-bold tracking-tight text-gray-900`)}>Запишитесь на экскурсию</h2>
        </div>
